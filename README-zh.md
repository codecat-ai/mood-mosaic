...
核心模型在 `src/journal.ts`，导入预览逻辑在 `src/importPreview.ts`，分析逻辑在 `src/summary.ts`，存储适配器在 `src/storage.ts`，React UI 在 `src/App.tsx`。

## 测试

测试覆盖校验和 upsert 行为、分析和马赛克排序、存储导入导出处理、导入预览、旧版 schema 规范化、按日期编辑，以及 UI 冒烟流程。修改行为前应先写测试。
